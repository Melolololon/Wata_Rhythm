...
void MelLib::SceneEditer::LoadRegisterSelectObject()
{
	...
	for (const auto& dirEntry : std::filesystem::directory_iterator("."))
	{
		...
		LoadName(file, data.typeName);
		LoadName(file, data.className);
...
void MelLib::SceneEditer::LoadName(std::ifstream& stream, std::string& str)
{
	while (1)
	{
		char c;
...
void MelLib::SceneEditer::LoadEditData(const std::string& sceneName)
{
	...
	while (1)
	{
		...
		LoadName(file, className);

		// オブジェクト名
		std::string objectName;
		LoadName(file, objectName);
		addObjectNames.push_back(objectName);

		// 登録名
		std::string regName;
		LoadName(file, regName);
